...
function functional_test_hammerdb {
  ...
  # Wait for the creator pod to run the create
  hammerdb_creator_pod=$(get_pod "app=hammerdb-bench-creator-$uuid" 300)
  kubectl wait --for=condition=Initialized "pods/$hammerdb_creator_pod" --namespace my-ripsaw --timeout=400s
  kubectl wait --for=condition=complete -l app=hammerdb-bench-creator-$uuid --namespace my-ripsaw jobs --timeout=500s

  # Wait for the workload pod to run the actual workload
  hammerdb_workload_pod=$(get_pod "app=hammerdb-bench-workload-mssql-$uuid" 300)
  kubectl wait --for=condition=Initialized "pods/$hammerdb_workload_pod" --namespace my-ripsaw --timeout=400s
  kubectl wait --for=condition=complete -l app=hammerdb-bench-workload-mssql-$uuid --namespace my-ripsaw jobs --timeout=500s
