...
function functional_test_uperf {
  ...
  wait_for "kubectl wait -n my-ripsaw --for=condition=Initialized pods/$uperf_client_pod --timeout=600s" "600s" $uperf_client_pod
  wait_for "kubectl wait -n my-ripsaw --for=condition=complete -l app=uperf-bench-client-$uuid jobs --timeout=600s" "600s" $uperf_client_pod
