...
The operator triggers roles based on the conditions defined in [cr](resources/crds/workload_all_cr.yaml) which will influence which roles the
...
Apply a new CR
```bash
# kubectl apply -f resources/crds/workload_all_cr.yaml
```
